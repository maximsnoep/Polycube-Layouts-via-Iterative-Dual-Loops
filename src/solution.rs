...
impl MeshResource {
    ...
    pub fn remove_path(&mut self, path_id: usize, configuration: &mut Configuration) -> bool {
        ...
        let singularities = self
            .get_top_n_percent_singularities((configuration.percent_singularities * 100.) as usize);
    ...
    pub fn add_loop(&self, configuration: &mut Configuration) -> Option<(Solution, Primalization)> {
        ...
        (subset_vertices, principal_direction) = if self.evaluation.face_to_fidelity.len() == 0 {
            ...
            (
                components[self.components_to_singularity_spread(
                    components.clone(),
                    principal_direction,
                    (configuration.percent_singularities * 100.) as usize,
        ...
        'outer: while let Some((score, (candidate_id, candidate_loop))) = candidate_loops.pop() {
            ...
            let singularities = self.get_top_n_percent_singularities(
                ((configuration.percent_singularities * 100.) as usize),
            );
